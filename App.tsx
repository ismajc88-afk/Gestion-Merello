    ...
    const renderContent = () => {
        switch (currentView) {
            ...
            case 'inventory': return <InventoryManager
                data={data}
                userRole={userRole}
                onUpdateBudget={(category, amount) => updateData({ budgetLines: data.budgetLines.map(l => l.category === category ? { ...l, estimated: amount } : l) })}
                onUpdateLimit={l => updateData({ budgetLimit: l })}
                onAddLine={(category, amount) => updateData({ budgetLines: [...data.budgetLines, { category, estimated: amount }] })}
                onDeleteLine={(category) => updateData({ budgetLines: data.budgetLines.filter(l => l.category !== category) })}
                onUpdateSubLines={(category, subLines) => updateData({ budgetLines: data.budgetLines.map(l => l.category === category ? { ...l, subLines } : l) })}
                onUpdateConfig={c => updateData({ appConfig: c })}
            />;
