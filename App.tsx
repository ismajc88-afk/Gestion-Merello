...
const App: React.FC = () => {
    ...
    const renderContent = () => {
        switch (currentView) {
            ...
            case 'stock': return <StockControl items={data.stock} categories={data.appConfig.stockCategories} categoryDefs={data.appConfig.stockCategoryDefs} units={data.appConfig.units} barSessions={data.barSessions} incidents={data.incidents} onUpdateStock={(id, q) => updateData({ stock: data.stock.map(i => i.id === id ? { ...i, quantity: q } : i) })} onAddItem={i => updateData({ stock: [...data.stock, { ...i, id: Date.now().toString(), lastUpdated: new Date().toISOString() }] })} onUpdateItem={(id, u) => updateData({ stock: data.stock.map(i => i.id === id ? { ...i, ...u } : i) })} onDelete={id => updateData({ stock: data.stock.filter(i => i.id !== id) })} onAddShoppingItems={items => { const newItems = items.map(item => ({ id: Date.now().toString() + Math.random().toString(36).slice(2, 7), name: item.name, quantity: item.quantity, unit: item.unit, checked: false, location: '', estimatedCost: item.estimatedCost, notes: item.notes, priority: 'HIGH' as const })); updateData({ shoppingList: [...data.shoppingList, ...newItems] }); toast.success(`${items.length} productos añadidos a la Lista de Compras`); }} />;
