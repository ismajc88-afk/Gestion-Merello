...
const App: React.FC = () => {
    ...
    const addAuditLog = useCallback((action: AuditActionType, module: string, detail: string) => {
        if (!userRole) return;
        ...
        updateData({ auditLog: [entry, ...(data.auditLog ?? [])].slice(0, 500) });
    }, [userRole, data.auditLog, updateData]);

    // --- ESTADOS CRÍTICOS ---
    const [alertState, setAlertState] = useState<{ active: boolean, title: string, msg: string, source: string } | null>(null);
    const [pushStatus, setPushStatus] = useState<{ status: 'IDLE' | 'SENDING' | 'SUCCESS' | 'ERROR', msg: string }>({ status: 'IDLE', msg: '' });
    ...
    const renderContent = () => {
        switch (currentView) {
            ...
            case 'cash': return <CashManager data={data} userRole={userRole} onAddTransaction={t => {
                updateData({ transactions: [...data.transactions, { ...t, id: Date.now().toString() }] });
                addAuditLog(t.type === 'EXPENSE' ? 'GASTO_AÑADIDO' : 'INGRESO_AÑADIDO', 'cash', `${t.description} — ${t.amount.toLocaleString()}€`);
            }} />;
            case 'inventory': return <InventoryManager data={data} userRole={userRole}
                onUpdateLimit={l => { updateData({ budgetLimit: l }); addAuditLog('PRESUPUESTO_MODIFICADO', 'inventory', `Límite global cambiado a ${l.toLocaleString()}€`); }}
                onUpdateBudget={(c, a) => { updateData({ budgetLines: data.budgetLines.map(l => l.category === c ? { ...l, estimated: a } : l) }); addAuditLog('PARTIDA_EDITADA', 'inventory', `Partida "${c}" ajustada a ${a.toLocaleString()}€`); }}
                onAddLine={(c, a) => { updateData({ budgetLines: [...data.budgetLines, { category: c, estimated: a }] }); addAuditLog('PARTIDA_CREADA', 'inventory', `Nueva partida "${c}" con límite ${a.toLocaleString()}€`); }}
                onDeleteLine={c => { updateData({ budgetLines: data.budgetLines.filter(l => l.category !== c) }); addAuditLog('PARTIDA_ELIMINADA', 'inventory', `Partida "${c}" eliminada`); }}
                onUpdateSubLines={(c, subs) => {
                    updateData({ budgetLines: data.budgetLines.map(l => l.category === c ? { ...l, subLines: subs } : l) });
                    addAuditLog('SUBPARTIDA_CREADA', 'inventory', `Subpartidas de "${c}" actualizadas`);
                }}
            />;
            ...
            case 'stock': return <StockControl items={data.stock} categories={data.appConfig.stockCategories} categoryDefs={data.appConfig.stockCategoryDefs} units={data.appConfig.units} barSessions={data.barSessions} incidents={data.incidents} onUpdateStock={(id, q) => { const item = data.stock.find(i => i.id === id); updateData({ stock: data.stock.map(i => i.id === id ? { ...i, quantity: q } : i) }); addAuditLog('STOCK_ACTUALIZADO', 'stock', `${item?.name || id}: cantidad → ${q}`); }} onAddItem={i => { updateData({ stock: [...data.stock, { ...i, id: Date.now().toString(), lastUpdated: new Date().toISOString() }] }); addAuditLog('PRODUCTO_AÑADIDO', 'stock', `Nuevo producto: ${i.name}`); }} onUpdateItem={(id, u) => { const item = data.stock.find(i => i.id === id); updateData({ stock: data.stock.map(i => i.id === id ? { ...i, ...u } : i) }); addAuditLog('PRODUCTO_EDITADO', 'stock', `${item?.name || id} editado`); }} onDelete={id => { const item = data.stock.find(i => i.id === id); updateData({ stock: data.stock.filter(i => i.id !== id) }); addAuditLog('PRODUCTO_ELIMINADO', 'stock', `Eliminado: ${item?.name || id}`); }} onAddShoppingItems={items => { const newItems = items.map(item => ({ id: Date.now().toString() + Math.random().toString(36).slice(2, 7), name: item.name, quantity: item.quantity, unit: item.unit, checked: false, location: '', estimatedCost: item.estimatedCost, notes: item.notes, priority: 'HIGH' as const })); updateData({ shoppingList: [...data.shoppingList, ...newItems] }); addAuditLog('PEDIDO_MÍNIMO', 'stock', `${items.length} productos añadidos a Lista de Compras`); toast.success(`${items.length} productos añadidos a la Lista de Compras`); }} onAutoExpense={expense => { updateData({ transactions: [...data.transactions, { id: Date.now().toString(), description: expense.description, amount: expense.amount, type: 'EXPENSE' as any, date: new Date().toISOString().split('T')[0], category: expense.category, subCategory: expense.subCategory }] }); addAuditLog('GASTO_AÑADIDO', 'stock', `Gasto automático: ${expense.description} (${expense.amount}€) → ${expense.category}${expense.subCategory ? ' / ' + expense.subCategory : ''}`); toast.success(`💰 Gasto de ${expense.amount}€ registrado en Tesorería → ${expense.category}`); }} onAutoIncome={income => { updateData({ transactions: [...data.transactions, { id: Date.now().toString(), description: income.description, amount: income.amount, type: 'INCOME' as any, date: new Date().toISOString().split('T')[0], category: income.category, subCategory: income.subCategory }] }); addAuditLog('INGRESO_AÑADIDO', 'stock', `Ingreso (devolución): ${income.description} (+${income.amount}€) → ${income.category}${income.subCategory ? ' / ' + income.subCategory : ''}`); toast.success(`♻️ Devolución de ${income.amount}€ ingresada en Tesorería → ${income.category}`); }} />;
            ...
            case 'shopping': return <ShoppingListManager items={data.shoppingList} budgetLines={data.budgetLines} locations={data.appConfig.locations} basicItems={data.appConfig.basicItems ?? []} onUpdateBasicItems={(items) => updateData({ appConfig: { ...data.appConfig, basicItems: items } })} onToggle={actions.handleShoppingToggle} onAdd={i => updateData({ shoppingList: [...data.shoppingList, { ...i, id: Date.now().toString(), checked: false }] })} onEdit={(id, u) => updateData({ shoppingList: data.shoppingList.map(i => i.id === id ? { ...i, ...u } : i) })} onDelete={id => updateData({ shoppingList: data.shoppingList.filter(i => i.id !== id) })} onAutoFill={() => {
                const basics = data.appConfig.basicItems ?? [
                    { name: 'Agua 1.5L', quantity: 20, unit: 'u', location: 'Barra' },
                    { name: 'Servilletas', quantity: 10, unit: 'u', location: 'Cocina' },
                ];
                if (basics.length === 0) { toast.info('No hay básicos configurados. Usa "Configurar Básicos".'); return; }
                let added = 0; let skipped = 0;
                const newItems: typeof data.shoppingList = [];
                basics.forEach(b => {
                    // Check if already in shopping list (unchecked)
                    if (data.shoppingList.some(s => s.name.toLowerCase() === b.name.toLowerCase() && !s.checked)) { skipped++; return; }
                    // Check stock: if item exists in stock and is above minimum, skip
                    const stockMatch = data.stock.find(s => s.name.toLowerCase().includes(b.name.toLowerCase().split(' ')[0]));
                    if (stockMatch && stockMatch.minStock > 0 && stockMatch.quantity > stockMatch.minStock) { skipped++; return; }
                    newItems.push({ ...b, id: Date.now().toString() + Math.random(), checked: false, isBought: false });
                    added++;
                });
                if (newItems.length > 0) updateData({ shoppingList: [...data.shoppingList, ...newItems] });
                if (added > 0 && skipped > 0) toast.success(`${added} añadidos, ${skipped} omitidos (stock OK o ya en lista)`);
                else if (added > 0) toast.success(`${added} básicos añadidos a la lista`);
                else toast.info(`Todo el stock está cubierto o ya está en la lista. Nada que añadir.`);
            }} />;
            ...
            case 'settings-master': return <AdminControlPanel data={data} kioskConfig={data.kioskConfig} onUpdateConfig={c => updateData({ appConfig: c })} onUpdateKioskConfig={c => updateData({ kioskConfig: c })} onResetModule={actions.resetModule} onFullImport={setData} peerCount={peerCount} onClearAuditLog={() => updateData({ auditLog: [] })} onHardReset={actions.resetFullApp} />;
            ...
            case 'kiosk': return <KioskMode
                ...
                userRole={userRole}
                initialMode={(userRole as string) === 'KIOSKO_CASAL' ? 'CASAL' : 'VENTA'}
                ticketCounts={getActiveSessionCounts()}
                mode={(userRole as string) === 'LOGISTICA' ? 'LOGISTICS' : ((userRole as string) === 'CAJERO' ? 'POS' : 'STOCK_ONLY')}
                onTicketSale={(items) => {
